...
class MetingJSElement extends HTMLElement {
  ...
  _init() {
    ...
    ((this.config = a),
      (this.api =
        ...
        "https://meting-api-rho-three.vercel.app/api?server=:server&type=:type&id=:id&r=:r"),
  ...
  _loadPlayer(a) {
    // 字段转换：API 返回 {title, author, pic, url, lrc} → APlayer 需要 {name, artist, cover, url, lrc}
    // 测试 API: https://meting-api-rho-three.vercel.app/api?server=netease&type=playlist&id=2842079283
    if (Array.isArray(a) && a.length > 0) {
      a.forEach(function (song) {
        // 统一封面字段：支持 cover 或 pic
        var cover = song.cover || song.pic;
        if (cover) {
          song.cover = cover;
        }
        // 统一歌名字段：支持 name 或 title
        if (!song.name && song.title) {
          song.name = song.title;
        }
        // 统一歌手字段：支持 artist 或 author
